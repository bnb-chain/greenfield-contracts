...
contract ERC1155Nontransferable is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    // Mapping from token id to token's URI
    // Used as theURI for token types by relying on ID substitution, e.g. https://token-cdn-domain/{id}.json
    ...
    function _setTokenURI(uint256 id, string memory newURI) internal {
        _tokenURI[id] = newURI;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (to != address(0)) {
            for (uint256 i; i < ids.length; ++i) {
                require(_balances[ids[i]][to] == 0 && amounts[i] == 1, "can only hold one token");
            }
        }
    }
