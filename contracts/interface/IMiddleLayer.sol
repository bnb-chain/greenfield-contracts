...
interface IMiddleLayer {
     ...
     */
    function handleAckPackage(uint8 channelId, uint64 sequence, bytes calldata msgBytes, uint256 _callbackGasLimit)
        external
     ...
     */
    function handleFailAckPackage(uint8 channelId, uint64 sequence, bytes calldata msgBytes, uint256 _callbackGasLimit)
        external
        returns (uint256 remainingGas, address refundAddress);
